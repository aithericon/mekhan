...
### Phase 4 — Governance: promote / demote / fork (decisions 4, 5) — ✅ DONE
1. **Promote** `POST /api/v1/templates/{id}/promote` body `{ origin?, coordinate, presentation }`. Role gate: workspace=Owner/Admin (`require_role` on the template's workspace). Sets `template_kind=library_node` + origin/coordinate/presentation/lifecycle across the WHOLE version family. *(done.)*
2. **Demote** `POST /api/v1/templates/{id}/demote` — reverse, role-gated; existing embeds frozen. *(done.)*
3. **Fork** `POST /api/v1/library/fork` body `{ coordinate }` (body-based — coordinates contain a slash) → deep-copy current version into a new editable `workspace` family, `forked_from` stamped. *(done.)*
4. **UI**: PromoteLibraryDialog (coordinate/vendor/category/icon/color/badge) + toolbar Promote/Manage/Demote (gated published && Admin+ && !system) + palette hover Fork. *(done.)*

> **✅ PHASE 4 COMPLETE 2026-06-14.** Commit `9d67005a`. **No migration** (all columns exist from P0/P1) and **no audit table** — the repo's template-governance convention is a structured `tracing` record + `updated_by` stamp (the only audit table is resource-specific). **v1 scope decision:** API promote supports `origin=workspace` only (gated on workspace Admin/Owner); `community` (platform-admin review queue) and `system` (seed-only) are rejected with clear errors — the codebase has **no platform-admin primitive**, so a half-gate would be dishonest. Added `GET /api/v1/node-library/categories` to keep the promote form's category picker in sync with the backend vocab (no FE drift). Fork is born `workspace`-visibility, NOT `private` (the `workflow_templates_private_owner_chk` constraint reserves `private` for owned sub-workflow children — caught live). `Presentation` now derives `Default`. Gates: governance unit tests 2/2, e2e `library-governance.test.ts` 2/2 + `library-palette.test.ts` 2/2, svelte 0/0, vitest 870, clippy clean (only pre-existing 13/12), openapi no-drift. **Live slot 6 (real backend :20600):** promote→library_node row; node-library lists it; re-promote (manage) 200; coordinate conflict 409; bad category/coordinate 400; demote→workflow + gone from palette; system demote 400; fork→new workspace template with forked_from + copied branding; fork unknown 404.

### Phase 5 — Upgrade prompt + lifecycle (decisions 2, 8, 11) — ✅ DONE
...
> **✅ PHASE 5 COMPLETE 2026-06-14.** **No migration** (all columns exist from P0/P1). Backend (`handlers/governance.rs`): `POST /api/v1/templates/{id}/lifecycle` (`{status, superseded_by?}`, Admin/Owner, library_node only, rejects `system`, stamps whole family) + `GET /api/v1/library/upgrade-preview?coordinate=&from=` (query-param coordinate because it contains a slash; resolves coordinate→family→latest, diffs `derive_child_io` on both versions, classifies `up_to_date`/`compatible`/`breaking`). Breaking iff a consumed input is removed/retyped or a new **required** input appears; output changes + new optional inputs are compatible. `node-library` descriptor gained `superseded_by` (palette shows `deprecated → succ`). Frontend: SubWorkflowSection upgrade prompt (compatible vs breaking, per-field "needs remap" badges; adopt = re-pin → existing contract effect reconciles + prunes mappings, so the remap finishes in the same fixed-row editor); PromoteLibraryDialog lifecycle section (deprecate/retire/reactivate + successor); TemplateEditor fork rebase hint (`forked_from` vs upstream latest); palette now fetches `include_deprecated=true` so deprecated nodes stay droppable with a warning (retired always server-excluded). **Gates:** governance unit 5/5 (coordinate/category/lifecycle validation + diff add-remove-retype + breaking-vs-compatible), e2e `library-governance` 3/3 (promote/lifecycle/fork) + `library-palette` 2/2, svelte 0/0, vitest 870, clippy `-D warnings` clean, openapi no-drift. **Live slot 6 (real backend :20600):** 9 lifecycle/guard flows correct (promote→deprecate+successor→node-library shows it→active-only hides it→retire hides everywhere→invalid-status 400→system-node 400→reactivate clears successor→demote); 2-version path proven end-to-end (promote v1, new_version+publish v2, re-promote → `upgrade-preview from=1` resolves `to=2, compatible`); `up_to_date` path correct. Migration renumbered `20240184`→**`20240185`** after merging main (main brought `20240184_seed_dev_user_two`). NOTE: `new_version` resets a draft to `template_kind=workflow` — re-promote after publishing the new version to re-list + trigger upgrade prompts (pre-P5 version-model behavior, not a P5 bug).
