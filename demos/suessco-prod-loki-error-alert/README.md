# Suessco Prod Loki Error-Log Alert

Scheduled error-log alert for the **prod** environment of the **Suessco** cluster,
driven by a **cross-cluster worker**. Identical to the `suessco-dev-loki-error-alert`
sibling except the routing group — the Loki query runs on an executor that lives
*inside the Suessco prod cluster* and enrolls into this mekhan's `suessco_prod`
worker pool, so it reaches Suessco prod's in-cluster Loki locally while the
control plane stays on aithericon.

```
Cron(12h) → Start{fire_time} → Loki query_range → Agent(summarize) → Decision(errors?)
            [group: suessco_prod ── runs on the Suessco prod worker]   ├─ yes → SMTP send → End
                                                                        └─ no  → End (no email)
```
...
2. **query_logs** — Loki `query_range` against the `suessco_loki` resource.
   ...
   mention it, bloating the token count). **`deploymentModel.group: "suessco_prod"`**
   pins this step to the executor enrolled in mekhan's `suessco_prod` worker pool —
   the one running inside the Suessco prod cluster — so `loki.service.consul:3100`
   resolves to *Suessco prod's* Loki, not dev's or aithericon's. The `limit` is
   deliberately small — the whole entry envelope feeds the Agent, and too many
   (or too large) entries overflow a 32K-token model context.
...
|------|---------|----------|-----------------|
| query_logs | `loki` | `suessco_loki` | `http://loki.service.consul:3100` resolved **inside the Suessco prod cluster** (via `group: suessco_prod`) |
...
- **`suessco_loki`** (`demos/resources/suessco_loki.json`) — Suessco's cluster
  ...
  `suessco_prod`-group worker). On a laptop, override `SUESSCO_LOKI_URL`.
...
`suessco_prod` worker pool from *inside* the Suessco prod cluster (see
`suessco/hetzner-cloud-cluster/environments/prod/09_mekhan_executor`). That
executor:
- dials OUT to aithericon for control (`mekhan.aithericon.eu`,
  `wss://nats.aithericon.eu`, S3), and
- reaches Suessco prod's services (Loki) locally.

Steps whose `deploymentModel.group = "suessco_prod"` are dispatched only to it.
...
```bash
mekhan apply demos/suessco-prod-loki-error-alert/
```

It is also seeded automatically at service startup (`MEKHAN__DEMOS__SEED=true`),
idempotent by templateId `00000000-0000-0000-0000-0000000000a3`.
...
- **Which cluster** — point this at a different downstream cluster by swapping
  the `group` value for that cluster's pool (here, `suessco_prod`).
