...
| ★ | `loki-error-alert/` | Ops capstone: cron trigger → Loki `query_range` → Agent summary → SMTP email → end. Scheduled error-log alert against the cluster Loki (`aithericon_loki` resource); applies from the CLI (no HumanTask sidecars). |
