...
use crate::port::{ResponseFormat, Role};
...
///
/// Deserialized from `ExecutionSpec.config` at runtime by `LlmBackend`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
// Custom serde for ResponseFormat: serialize as tagged enum
        ...
        match raw.r#type.as_str() {
            ...
            "json_schema" => {
                let schema = raw.schema.ok_or_else(|| {
                    serde::de::Error::custom("json_schema response_format requires a non-null output_schema (schema field)")
                })?;
