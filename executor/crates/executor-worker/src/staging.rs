...
pub fn default_pipeline(
    ...
    // FetchConfigHook MUST run before InjectSecretsHook — secret resolution
    // scans `spec.config` for `{{secret:KEY}}` refs, and the config isn't
    // present yet on `config_ref`-shaped jobs.
    if let Some(s) = store.clone() {
        pipeline = pipeline.add_hook(FetchConfigHook { store: s });
    }

    // Secrets AFTER environment injection + config fetch, BEFORE inputs staging
