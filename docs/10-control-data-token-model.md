...
**One invariant core, two resolver adapters.** The producer-borrow **location invariants** — known slug, no self-reference, producer upstream of consumer, producer parks its output — are single-sourced in `compiler/borrow/resolve_core.rs` (`producer_for_slug` + `check_borrowable_producer`). The upstream check runs under one of two rules: `StrictTopo` (plain topo-position compare; backend `{{slug.field}}` refs) or `GuardReachability` (strict topo **or** LeaseScope-containment recovery **or** the Loop body-child exception; guard refs). Two adapters sit on the core — `resolve_ref()` for Rhai-source guard refs, `resolve_backend_ref()` (plus the Envelope planner arm) for backend placeholder refs. They stay adapters because field validation is shape-context-specific (full `TokenShape` model vs. flat port decls) and failure mapping differs (`Unresolved`/`GuardUnresolved` vs. `BackendRef*` hard errors vs. silent Envelope skip), but the invariants themselves cannot drift.

`guard_refs()` (raw `scan_dotted_refs` scanner + `rhai_scope::extract_qualified_refs` gating to exclude Rhai locals/keywords/strings/comments) → `resolve_ref()`. This resolver still feeds:
...
| One invariant core, two resolver adapters | location invariants: `check_borrowable_producer()` — `borrow/resolve_core.rs`; guard adapter `guard_refs()` → `resolve_ref()` (consumers `reachable_scope()`, `check_guard()`, `guard_readarc_plan()`); backend adapter `resolve_backend_ref()` |
