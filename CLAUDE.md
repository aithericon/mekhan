...
- **Worktrees share cargo target dirs by default.** Each fresh worktree would otherwise accumulate ~80–230 GB across the three workspaces. The `.envrc` files (umbrella, `engine/`, `executor/`) — tracked, portable via literal `$HOME` — export `CARGO_TARGET_DIR=$HOME/.cache/cargo-targets/aithericon-platform/{umbrella,engine,executor}/` via direnv. `just dev::worktree-add <name>` creates a worktree, runs `just/scripts/setup-shared-target.sh` inside it (idempotent — writes any missing `.envrc` files and `direnv allow`s all three), and reports the path. The umbrella's `.envrc` contains `use flake` alongside a marker-delimited shared-target block so the script only touches its own region. Trade-off: cargo serializes builds per target dir, so simultaneous `cargo build` in two worktrees waits on the lock — running/testing/editing in parallel is unaffected. The tracked `.cargo/config.toml` is unchanged (still owns the cross-musl linker setup). To migrate an existing worktree's `target/` into the shared dir, run `just dev::setup-shared-target --migrate` (preserves cache, instant `mv` since same filesystem) or `--clean` (deletes).
- **Rust-analyzer phantom errors after switching to shared targets.** rust-analyzer spawned by Claude Code (or any IDE/LSP) inherits the **parent process's** env, not direnv's — so a long-lived LSP started before this setup keeps building into the worktree-local `./target/` and reports phantom errors against stale artifacts even though `cargo check` (running with direnv-loaded env) is clean. Fix is in `.claude/settings.local.json`'s `env.CARGO_TARGET_DIR` (set to the umbrella shared dir) — applies only to **new** Claude sessions. Sequence to clear an existing case: (a) verify with `direnv exec . cargo check`, (b) `rm -rf ./target` in the affected worktree, (c) restart the Claude session (or `kill <rust-analyzer pid>` to force respawn under the new env). Note that `.claude/settings.local.json` is gitignored, so the absolute path baked in there is per-machine.
