...
# NOTE: the resource ADDRESS stays `mekhan_dev` in both layers — renaming it
# would make Terraform drop + recreate the live database (no prevent_destroy
# here). The DB/role NAME is env-derived (local.db_name → mekhan_dev /
# mekhan_prod), so prod creates `mekhan_prod` from this same block.
