#!/usr/bin/env bash
# =============================================================================
# Bootstrap or rotate the mekhan-<env> NATS account + user on the shared
# HetznerCluster NATS broker.    Usage: ./generate-nats-user.sh [dev|prod]
# =============================================================================
# Mints a single user (`mekhan-<env>-worker`) under the `mekhan-<env>` account
# and publishes its .creds bundle to Vault at
# secret/nats/apps/mekhan/<env>/worker. Mekhan's Nomad jobspec
# (mekhan.nomad.hcl.tpl) reads that path via a `template` stanza at alloc
# start. The matching Vault policy + JWT auth role are owned by vault.tf —
# `tofu apply` creates them, this script populates the secret they grant
# access to. Run it ONCE PER ENV before that env's first `tofu apply`.
#
# When to run:
...
# Account + user identifiers — env-scoped so dev and prod get separate NATS
# accounts (and therefore separate JetStream + subject namespaces) on the one
# shared broker. ENV defaults to "dev"; pass `prod` (or set ENV=prod) to mint
# the prod account. Keep the derived names in sync with locals.tf
# (nats_account_name / nats_user_name / nats_user_kv_path) and the Vault role's
# bound_claims in vault.tf.
ENV="${1:-${ENV:-dev}}"
case "${ENV}" in
  dev | prod) ;;
...
ACCOUNT_NAME="mekhan-${ENV}"
USER_NAME="mekhan-${ENV}-worker"
ACCOUNT_VAULT_PATH="secret/nats/apps/mekhan/${ENV}/account"
USER_VAULT_PATH="secret/nats/apps/mekhan/${ENV}/worker"
