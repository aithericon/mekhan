...
deploy-prod ──►   ▼ tofu apply -var=image_tag=$SHA        │
              ...
              │  - provider="consul" + Traefik  │         │  /api/v1/health → HTTP check
...
| Nomad deployment fails health check | `/api/v1/health` not responding — check `nomad alloc logs <alloc-id>`. Common cause: `MEKHAN__DATABASE_URL` wrong, or Postgres firewall doesn't allow the Nomad client subnet. |
