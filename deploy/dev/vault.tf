...
locals {
  # Resource-store path layout. Keep in sync with:
  ...
  # Policy / role names. Two roles total — one per Nomad job.
  vault_policy_nats_read     = "mekhan-nats-read"
  vault_policy_resources_rw  = "mekhan-resources-rw"
  vault_policy_wrap          = "mekhan-wrap"
  vault_role_mekhan_service  = "mekhan-service"
  vault_role_mekhan_executor = "mekhan-executor"
}
...
resource "vault_jwt_auth_backend_role" "mekhan_service" {
  backend   = "jwt-nomad"
  role_name = local.vault_role_mekhan_service
  role_type = "jwt"

  bound_audiences = ["vault.io"]
  bound_claims = {
    nomad_namespace = var.nomad_namespace
    nomad_job_id    = "mekhan-service"
  }
...
resource "vault_jwt_auth_backend_role" "mekhan_executor" {
  backend   = "jwt-nomad"
  role_name = local.vault_role_mekhan_executor
  role_type = "jwt"

  bound_audiences = ["vault.io"]
  bound_claims = {
    nomad_namespace = var.nomad_namespace
    nomad_job_id    = "executor"
  }
