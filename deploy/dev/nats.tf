...
# either the `mekhan-service` (BFF + co-located engine task) or `executor`
# job presents its JWT to Vault's `jwt-nomad` backend, Vault checks the
# bound_claims; if they match, it
