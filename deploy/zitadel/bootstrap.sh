...
# NOTE: the introspection API app + token-broker service user that used to be
# provisioned here were retired when Mekhan moved to native `uat_` PATs (the
# `user_pats` table; no Zitadel introspection / Management-API broker). Zitadel
# is now a login-only IdP, so BFF bootstrap only needs the OIDC app above.
...
GitOps machine token (CI \`mekhan apply\`):
  ...
  Woodpecker secret (a mekhan-native \`uat_\` PAT). Revoke it from the same
  page; the revocation takes effect immediately.
────────────────────────────────────────────────────────────
EOF
