//! Layer 0: Invoice Processing Orchestrator — full-feature demo with conditional routing.
//!
...
//! 7. **Multi-storage I/O** — per-input S3 download, per-output S3 upload (no file-ops steps)
...
//!
//! ```text
//! Layer 0: orchestrator-net   (this net — human tasks + timer + sequencing)
...
//!
//! Control tokens drive the pipeline; data tokens are parked and read via take+put-back arcs.
//!
//! ```text
//! Parked data tokens:
//!   [entry_data]        ← parse_entry (InvoiceParams: department, urgency, notes, document_key, document_type)
//!   [extracted_data]    ← join_ocr | join_parse (ExtractedInvoice: invoice_data JSON)
//!   [validation_result] ← join_validation (ValidationResult: validated_data, risk_flags, compliance_status, ...)
//!
//! Control flow:
//!   [entry_form] → (request_entry) → [entry_task] + [sig_entry_response]
//!   [entry_task] + [sig_entry_response] → (parse_entry) → [invoice_params] + [entry_data]
//!
//!   Conditional routing (MIME type guard on invoice_params.document_type):
//!     IMAGE: [invoice_params] + [entry_data]↺ → (prepare_ocr) → ... → (join_ocr) → [data_extracted] + [extracted_data]
//!       (executor stages input from S3 via per-input storage, resolves path via {{input_path:NAME}})
//!     DOC:   [invoice_params] + [entry_data]↺ → (prepare_extract) → ... → (join_extract) → [extract_done]
//!       (executor stages input from S3 via per-input storage)
//!            [extract_done] + [entry_data]↺ → (prepare_parse) → ... → (join_parse) → [data_extracted] + [extracted_data]
//!
//!   [data_extracted] + [extracted_data]↺ + [entry_data]↺ → (prepare_validation) → ... → (join_validation) → [validation_done] + [validation_result]
//!     (validation outputs uploaded to S3 via per-output upload_to)
//!   [validation_done] + [validation_result]↺ → (prepare_summary) → ... → (join_summary) → [summary_done]
//!   [summary_done] + [validation_result]↺ + [entry_data]↺ → (prepare_review) → [review_form]
//!
//!   [review_form] → (request_review) → [review_task] + [sig_review_response]
//!   Race: human_approved | human_rejected | sla_breach → terminal places
//!
//!   ↺ = take + put-back (read arc)
//! ```
//!
//! ## Deploy
//!
...
// EntryForm replaced by HumanTaskRequest from SDK (the handler contract IS the token type).
// EntryTask replaced by HumanTaskAssigned from SDK effect_tokens.
...
/// Step ready to be dispatched as a job (control token).
#[token]
struct StepReady {
    job_id: String,
    model_name: String,
...
/// Pending step — held while waiting for job-net result (control token).
#[token]
struct StepPending {
    job_id: String,
    phase: String,
...
/// The handler's merge pattern preserves extra fields in the output token.
#[token]
struct ReviewForm {
    title: String,
...
fn definition(ctx: &mut Context) {
    ...
    let (invoice_params, entry_data) = ctx.scope("1. Data Entry", |ctx| {
        ...
        ctx.seed(
            ...
            vec![HumanTaskRequest {
                ...
                process_step: None,
                steps: vec![
                    TaskStep::new("invoice", "Invoice Details")
                        .description("Upload the invoice document for processing.")
                        .input(
                            TaskField::file("invoice_file", "Invoice Document")
                                .required()
                        ...
                        .divider()
                        .input(
                            TaskField::select("department", "Department")
                                .required()
                                .options(&["engineering", "marketing", "operations", "finance", "hr"]),
                        )
                        .input(
                            TaskField::select("urgency", "Processing Urgency")
                                .required()
    ...
    let (data_extracted, extracted_data) = ctx.scope("2. Data Extraction", |ctx| {
        ...
        ctx.scope("OCR (Images)", |ctx| {
            let ocr_ready = ctx.state::<StepReady>("ocr_ready", "OCR Job Ready");
            let ocr_pending = ctx.state::<StepPending>("ocr_pending", "OCR Pending");

            ctx.transition("prepare_ocr", "Prepare OCR Job (Image)")
            ...
            ctx.transition("dispatch_ocr", "Dispatch OCR Job")
                .auto_input("step", &ocr_ready)
                .auto_output("req", &to_jobs)
                .auto_output("pending", &ocr_pending)
                .logic(
                    r#"#{
                        req: #{
                            job_id: step.job_id,
            ...
            ctx.transition("join_ocr", "Join OCR Result")
                .process_step_completed("extraction")
                .read_input("process", &processes)
                .auto_input("result", &result_inbox)
                .auto_input("pending", &ocr_pending)
        ...
        let extract_done = ctx.scope("Kreuzberg (Documents)", |ctx| {
            let extract_ready = ctx.state::<StepReady>("extract_ready", "Extract Job Ready");
            let extract_pending = ctx.state::<StepPending>("extract_pending", "Extract Pending");
            let extract_done = ctx.state::<ExtractDone>("extract_done", "Extract Done");

            ctx.transition("prepare_extract", "Prepare Extract Job (Document)")
                .process_step_started("extraction")
                .read_input("process", &processes)
                .read_input("entry", &entry_data)
                .auto_input("params", &invoice_params)
                .guard(r#"!params.document_type.starts_with("image")"#)
                .auto_output("ready", &extract_ready)
                .logic(
                    r#"
            ...
            ctx.transition("dispatch_extract", "Dispatch Extract Job")
                .auto_input("step", &extract_ready)
                .auto_output("req", &to_jobs)
                .auto_output("pending", &extract_pending)
                .logic(
                    r#"#{
                        req: #{
                            job_id: step.job_id,
            ...
            ctx.transition("join_extract", "Join Extract Result")
                .auto_input("result", &result_inbox)
                .auto_input("pending", &extract_pending)
                .correlate("result", "pending", "job_id")
            ...
            ctx.transition("fail_extract", "Fail Extract Step")
                .auto_input("fail", &failure_inbox)
                .auto_input("pending", &extract_pending)
                .correlate("fail", "pending", "job_id")
                .auto_output("out", &workflow_failed)
        ...
        ctx.scope("Text Parse", |ctx| {
            let parse_ready = ctx.state::<StepReady>("parse_ready", "Parse Job Ready");
            let parse_pending = ctx.state::<StepPending>("parse_pending", "Parse Pending");

            ctx.transition("prepare_parse", "Prepare Parse Job")
                .read_input("entry", &entry_data)
                .auto_input("data", &extract_done)
                .auto_output("ready", &parse_ready)
                .logic(
                    r#"
            ...
            ctx.transition("dispatch_parse", "Dispatch Parse Job")
                .auto_input("step", &parse_ready)
                .auto_output("req", &to_jobs)
                .auto_output("pending", &parse_pending)
                .logic(
                    r#"#{
                        req: #{
                            job_id: step.job_id,
            ...
            ctx.transition("join_parse", "Join Parse Result")
                .process_step_completed("extraction")
                .read_input("process", &processes)
                .auto_input("result", &result_inbox)
                .auto_input("pending", &parse_pending)
                .correlate("result", "pending", "job_id")
                .auto_output("invoice", &extracted_data)
                .auto_output("ctrl", &data_extracted)
                .logic(
            ...
            ctx.transition("fail_parse", "Fail Parse Step")
                .auto_input("fail", &failure_inbox)
                .auto_input("pending", &parse_pending)
                .correlate("fail", "pending", "job_id")
                .auto_output("out", &workflow_failed)
    ...
    let (validation_done, validation_result) = ctx.scope("3. Validation", |ctx| {
        ...
        let validation_ready = ctx.state::<StepReady>("validation_ready", "Validation Job Ready");
        let validation_pending =
            ctx.state::<StepPending>("validation_pending", "Validation Pending");

        ctx.transition("prepare_validation", "Prepare Validation Job")
            .process_step_started("validation")
            .read_input("process", &processes)
            .read_input("invoice", &extracted_data)
            .read_input("entry", &entry_data)
            .auto_input("ctrl", &data_extracted)
            .auto_output("ready", &validation_ready)
            .logic(format!(
                r#"
            ...
            "#,
                script_json = serde_json::to_string(validation_script).unwrap()
            ));

        ctx.transition("dispatch_validation", "Dispatch Validation Job")
            .auto_input("step", &validation_ready)
            .auto_output("req", &to_jobs)
            .auto_output("pending", &validation_pending)
            .logic(
                r#"#{
                    req: #{
                        job_id: step.job_id,
        ...
        ctx.transition("join_validation", "Join Validation Result")
            .process_step_completed("validation")
            .read_input("process", &processes)
            .auto_input("result", &result_inbox)
            .auto_input("pending", &validation_pending)
            .correlate("result", "pending", "job_id")
        ...
        ctx.transition("fail_validation", "Fail Validation Step")
            .auto_input("fail", &failure_inbox)
            .auto_input("pending", &validation_pending)
            .correlate("fail", "pending", "job_id")
            .auto_output("out", &workflow_failed)
    ...
    let summary_done = ctx.scope("4. Summary", |ctx| {
        ...
        let summary_ready = ctx.state::<StepReady>("summary_ready", "Summary Job Ready");
        let summary_pending = ctx.state::<StepPending>("summary_pending", "Summary Pending");

        ctx.transition("prepare_summary", "Prepare Summary Job")
            .process_step_started("summary")
            .read_input("process", &processes)
            .read_input("vresult", &validation_result)
            .auto_input("ctrl", &validation_done)
            .auto_output("ready", &summary_ready)
            .logic(
                r#"
        ...
        ctx.transition("dispatch_summary", "Dispatch Summary Job")
            .auto_input("step", &summary_ready)
            .auto_output("req", &to_jobs)
            .auto_output("pending", &summary_pending)
            .logic(
                r#"#{
                    req: #{
                        job_id: step.job_id,
        ...
        ctx.transition("join_summary", "Join Summary Result")
            .process_step_completed("summary")
            .read_input("process", &processes)
            .auto_input("result", &result_inbox)
            .auto_input("pending", &summary_pending)
            .correlate("result", "pending", "job_id")
        ...
        ctx.transition("fail_summary", "Fail Summary Step")
            .auto_input("fail", &failure_inbox)
            .auto_input("pending", &summary_pending)
            .correlate("fail", "pending", "job_id")
            .auto_output("out", &workflow_failed)
...
fn main() {
    aithericon_sdk::run(
        ...
        "Invoice Processing Pipeline: Vision OCR / Kreuzberg + Python validation + file-ops + LLM + human review + timer",
        definition,
    );
}
