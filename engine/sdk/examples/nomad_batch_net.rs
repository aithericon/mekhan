...
//!
//! [sig_running]   (signal place — NomadWatcher delivers "running" here)
//! [sig_completed] (signal place — NomadWatcher delivers "completed" here)
//! [sig_failed]    (signal place — NomadWatcher delivers "failed" here)
//!
...
//! ```
//!
//! Quick start (self-contained demo with local Nomad dev agent):
//! ```bash
//! just nomad-demo
//! ```
//!
//! Manual execution against a real Nomad cluster:
//! ```bash
//! SCHEDULER_BACKEND=nomad
//! SCHEDULER_JOB_TEMPLATE=petri-batch-worker
...
fn definition(ctx: &mut Context) {
    ...
    // 1. submit_job — effect transition dispatching to Nomad via scheduler_submit.
    ...
    // 2. t_running — signal join: NomadWatcher delivers a "running" signal.
    ...
    // 3. t_success — signal join: NomadWatcher delivers a "completed" signal.
    ...
    // 4. t_failed — signal join: NomadWatcher delivers a "failed" signal.
    ...
    // 5. retry — re-queue failed job with incremented run epoch and retry count.
    ...
    // 6. dead_letter — retries exhausted, move to terminal dead-letter place.
    ...
    // 7–8. Effect error recovery — retry + DLQ with custom dead-letter fields.
    ...
    // 9. cancel_submitted — cancel a job that has been submitted but not yet running.
    ...
    // 10. cancel_running — cancel a job that is currently running.
