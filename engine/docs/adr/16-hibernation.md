...
- **Cold-start latency.** Waking a hibernated net requires rehydrating from the NATS event log (or snapshot). This adds latency to the first signal after idle. Snapshotting (ADR-11) mitigates this but is a prerequisite for production use.
