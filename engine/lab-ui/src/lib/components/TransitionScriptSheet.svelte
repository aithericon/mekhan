<script lang="ts">
	...
	import type { Transition, Port, TransitionStatus } from '$lib/api/client';
	...
	let { transition, inputPorts, outputPorts, guard, script, status, effectHandlerId = null, open, onClose }: Props = $props();
	...
	async function saveChanges() {
		...
		try {
			const base = store?.apiBase ?? '/api';
			const response = await fetch(`${base}/topology/transition/${transition.id}`, {
				method: 'PATCH',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
					script: editedScript,
					guard: editedGuard || undefined
				})
			});
			...
			if (response.ok && data.success) {
				// Refresh topology FIRST to get updated scripts
				await store?.fetchTopology();
				// Refresh state to get updated transition statuses
				await store?.fetchEvents();
				// Exit edit mode AFTER fetch completes so the new script value is available
				isEditing = false;
			} else {
				saveError = data.error || 'Failed to save changes';
			}
		} catch (error) {
			saveError = error instanceof Error ? error.message : 'Network error';
		} finally {
			isSaving = false;
		}
	}
	...
	const statusText = $derived.by(() => {
		...
		switch (status.status) {
			...
			case 'disabled_guard_error':
				return { text: `Disabled: Guard error - ${status.error}`, color: 'text-red-600 bg-red-50' };
			default:
				return null;
		}
	});
				...
				{#if guard}
					<div>
						<h3 class="text-sm font-medium text-foreground/80 mb-2 flex items-center gap-2">
							Guard Script
							{#if status?.status === 'disabled_guard_failed'}
			...
			{:else}
				...
				{#if guard || isEditing}
					<div>
						<h3 class="text-sm font-medium text-foreground/80 mb-2 flex items-center gap-2">
							Guard Script
							{#if !isEditing && status?.status === 'disabled_guard_failed'}
								<span class="text-xs text-red-500">(currently false)</span>
							{/if}
						</h3>
						{#if isEditing}
