...
async fn meta_filters_and_sort_via_list_entries() {
    ...
    let page = list_entries(
        &pool,
        Uuid::nil(),
        &scoped(&exec, "filter[meta.num_rows][gte]=100"),
    )
    .await
    ...
    let err = list_entries(
        &pool,
        Uuid::nil(),
        &scoped(&exec, "filter[meta.bogus][eq]=x"),
    )
    .await
    .unwrap_err();
    assert!(
...
async fn facets_bucket_math_exact() {
    ...
    // --- format ---------------------------------------------------------------
    let by_format = facets(
        &pool,
        Uuid::nil(),
        &params,
        CatalogueDimension::Format,
        limit,
    )
    .await
