...
fn every_numbered_demo_has_a_snapshot_test_or_is_documented_skip() {
    ...
    // 21-asset-consume / 22-asset-ref / 23-resource-ref are the named-global demos:
    // each references a scope-visible asset/resource by ref-key that only the
    // publish handler's `discover_named_globals` resolver can supply (bare
    // `compile_to_air` has an empty KnownGlobals, so the asset staging / object-asset
    // constant-inline / resource public-field constant-inline can't resolve). Their
    // compile + inline behavior is proven live by the publish path + `mekhan test`.
