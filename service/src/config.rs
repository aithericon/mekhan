...
pub struct AuthConfig {
    #[serde(default)]
    pub mode: AuthMode,
    /// Required when `mode = "zitadel"`. The Zitadel issuer URL
    /// (e.g. `https://your-instance.zitadel.cloud`).
    #[serde(default)]
    pub issuer_url: Option<String>,
    /// Required when `mode = "zitadel"`. The OIDC `aud` claim value Mekhan
    /// was registered as in Zitadel.
    #[serde(default)]
    pub audience: Option<String>,
    /// Comma-separated list of permitted CORS origins. Required for browsers
    /// to send the Authorization header on cross-origin preflight.
    #[serde(default)]
...
pub enum AuthMode {
    /// Validate Bearer JWTs against a Zitadel issuer.
    Zitadel,
    /// Accept any (or no) token and inject a fixed dev user. **Never** boot
    /// production with this — `main.rs` refuses the combination.
    #[default]
    DevNoop,
}
