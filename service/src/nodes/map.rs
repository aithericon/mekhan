...
fn input_ports(_data: &WorkflowNodeData) -> Vec<Port> {
    // Map accepts the outer `in` and a `body_out` handle from its body
    // children (their terminal edge targets `body_out`). Both are Json
    // pass-throughs — mirrors Loop's input ports.
    vec![
        Port::empty_input(),
        Port {
