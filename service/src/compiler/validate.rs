...
/// an INLINE AutomatedStep on an `ExecutorJob` backend (the executor lifecycle
/// preserves `_`-leaves + parks declared outputs), an Agent (degenerate or
/// full-loop — both emit the executor-shaped envelope), or a SubWorkflow
/// (the lowering re-shapes its join into a `detail.outputs` envelope and threads
/// the `__map_*` correlation leaves through the child via the spawn `initial_token`
/// → reply round-trip — verbatim, per the engine bridge, no shared side place).
...
fn map_body_terminal_supported(data: &WorkflowNodeData) -> bool {
    match data {
        WorkflowNodeData::AutomatedStep {
            ...
            matches!(
                deployment_model,
                crate::models::template::DeploymentModel::Inline
