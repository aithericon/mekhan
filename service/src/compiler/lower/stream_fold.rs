//! `WorkflowNodeData::StreamFold` lowering — drain a streaming producer's
//! mid-execution output and fold it into ONE output token, gated behind an
//! end-of-stream counted barrier so the net WAITS for the full stream before
//! completing. No body, no executor — the fold is pure Rhai in the net.
//!
//! This is the clean extraction of the old `StreamConsumer`'s default `Rhai`
//! dispatch (the body-dispatch streaming patterns moved elsewhere: per-chunk
//! parallel map → a streaming-source `Map`; stateful in-process reduce → an
//! `AutomatedStep` with `streamInput`).
//!
...
//! At producer job-end the executor's terminal `Completed` status carries
//! `stream_count` = the number of distinct outputs; `split_outputs_streaming`
//! surfaces it as a TOP-LEVEL leaf on the producer's slim control token.
//!
...
//!     EOS/completion token carrying `stream_count`).
...
pub(crate) fn lower_stream_fold(cx: &mut LoweringCtx) -> Result<(), CompileError> {
    let id = &cx.node.id;
    let WorkflowNodeData::StreamFold {
        ...
        unreachable!("lower_stream_fold on non-StreamFold node")
    };
    ...
    // orders on them exactly as Map's per-element results do. Pure Rhai
    ...
    // surfaces it there). If absent (a non-streaming producer mis-wired into
    // the control handle) default to 0 so the gather fires immediately on an
    // empty stream rather than wedging.
    ...
    // edge to the place keyed by its `targetHandle`.
    ...
    // StreamFold is a parked producer: the reduced output is stored write-once
    // at `p_<id>_data`, schemed `Data__<id>` by the foundation pass.
