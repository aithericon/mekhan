...
/// Generate a `_aithericon_io.py` module: a frozen `Input` dataclass whose
/// fields mirror the node's input scope, plus a self-contained `load_input()`.
/// The loader prefers the SDK (`aithericon.token()`) and falls back to reading
/// the staged `input.json`, so it works even when the SDK isn't installed.
pub fn generate_py_io_module(
    ...
    if decls.is_empty() {
        decls.push_str("    # no typed upstream fields — this step's token is pass-through\n");
    }

    format!(
        r#"# Generated by Aithericon — do not edit.
# Typed view of this step's input token. Fields mirror the upstream scope
# (the same `input.<field>` model decision guards use). Regenerated on every
# publish from the live graph.
...
class Input:
{decls}    raw: dict = field(default_factory=dict)

    @classmethod
    def from_token(cls, token: dict) -> "Input":
        return cls(
{assigns}            raw=token,
        )
...
def load_input() -> Input:
    """This step's input token as a typed `Input`."""
    return Input.from_token(_load_token())
"#,
        decls = decls,
        assigns = assigns
    )
