...
pub(crate) fn build_retry_topology(
    ...
    ctx.transition("on_failed", "On Failed")
        ...
        .logic(
            ...
            #{
                f: #{ job_id: e.job_id, run: e.run, retries: e.retries, max_retries: e.max_retries, spec: e.spec, executor_namespace: e.executor_namespace, reason: "failed" },
    ...
    ctx.transition("on_timeout", "On Timeout")
        ...
        .auto_output("log", &failure_log)
        .logic(
            r#"#{ f: #{ job_id: e.job_id, run: e.run, retries: e.retries, max_retries: e.max_retries, spec: e.spec, executor_namespace: e.executor_namespace, reason: "timed_out" }, log: #{ level: "error", source: "executor", message: "Automated step timed out", detail: #{ execution_id: e.execution_id, run: e.run, retries: e.retries } } }"#,
        );
    ...
    // `executor_namespace` MUST be carried forward (`()` → null → submit
    // handler's default when a step has none): without it the resubmit falls
    // back to the bare `executor` namespace that no group consumer drains, so
    // the retry is black-holed and the instance hangs (the group-partitioned
    // dispatch landed there after the unified-worker change).
    let resubmit = r#"#{ job_id: f.job_id, run: f.run + 1, retries: f.retries + 1, max_retries: f.max_retries, spec: f.spec, executor_namespace: f.executor_namespace }"#;
