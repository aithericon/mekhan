<script lang="ts">
	// Top-level asset browser (docs/20 §3/§4). Two sibling tabs:
	//   1. ASSETS — typed collections (a flat list within the active scope).
	//   2. Asset TYPES — the user-defined schemas (define / edit / delete).
	// The active scope (workspace or a real folder, driven by the parent's
	// folder rail / the built-in selector) is both the owner-scope for new
	// types/assets and the placement filter. Real folders replaced the old
	// `display_path` virtual-folder string, so there is no in-list grouping.
	//
...
<div class="space-y-4" data-testid="assets-list">
	...
	<Tabs.Root value={activeTab} onValueChange={(v) => (activeTab = v === 'types' ? 'types' : 'assets')}>
		...
		<!-- Assets (collections) — flat list within the active scope -->
