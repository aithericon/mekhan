<script lang="ts">
	...
	// This component owns ONE template's IDE session for its whole lifetime.
	// The route wraps it in `{#key templateId}`, so an in-app nav to another
	// version destroys it (binding.destroy + releaseSession → WS close) and
	// mounts a fresh instance — same init path as a cold load.
	...
	// Yjs session — bound once for this component instance; the `{#key}`
	// wrapper remounts it on id change, so the initial-value read is intended.
	...
	async function load() {
		try {
			...
			// refreshScopes() call fires before load() completes (both run
			// concurrently on mount), so that call has no workspace_id and
			// returns an empty Globals group. Calling again here ensures
	...
	async function handleNewVersion() {
		...
		try {
			...
			// mounts the fresh draft's IDE — no full document load needed.
			await goto(`/templates/${next.id}/ide`);
		} catch (e) {
