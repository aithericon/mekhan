...
class StepperNavState {
	constructor(
		readonly opts: StepperNavProps,
		readonly rootState: StepperRootState
	) {}

	currentStep = $derived.by(() => this.rootState.opts.step.current);
...
class StepperItemState {
	...
	isCurrent = $derived.by(() => {
		return this.step === this.rootState.opts.step.current;
	});

	state: 'active' | 'completed' | 'inactive' = $derived.by(() => {
		if (this.rootState.opts.completed.current) return 'completed';
	...
	props = $derived.by(() => ({
		...
		'data-current': this.isCurrent ? 'true' : 'false'
	}));
}
...
class StepperItemTriggerState {
	...
	props = $derived.by(() => ({
		...
		'data-current': this.itemState.isCurrent ? 'true' : 'false',
...
export function useStepperNav(props: StepperNavProps) {
	return StepperNavCtx.set(new StepperNavState(props, StepperCtx.get()));
}
...
export function useStepperIndicator() {
	return StepperItemCtx.get();
}
