...
describe('YjsGraphBinding', () => {
	...
	describe('resources (workflow-level alias → type)', () => {
		it('initial state has no resources field', () => {
			// Empty Y.Map("resources") materialises as an absent field on
			// the graph — matches the schema's optional `resources?:` and
			// keeps `graph.resources` falsy on legacy docs.
			expect(binding.graph.resources).toBeUndefined();
		});

		it('setResource declares an alias', () => {
			binding.setResource('db', 'postgres');
			expect(binding.graph.resources).toEqual({ db: 'postgres' });
		});

		it('aliases materialise in alphabetical order', () => {
			binding.setResource('zeta', 'slack');
			binding.setResource('alpha', 'openai');
			binding.setResource('mid', 'postgres');
			// `BTreeMap` on the Rust side is alphabetised — the binding
			// matches so the rendered picker order is stable and
			// equivalent to the wire / YAML order.
			expect(Object.keys(binding.graph.resources!)).toEqual(['alpha', 'mid', 'zeta']);
		});

		it('removeResource drops the alias', () => {
			binding.setResource('db', 'postgres');
			binding.setResource('ai', 'openai');
			binding.removeResource('db');
			expect(binding.graph.resources).toEqual({ ai: 'openai' });
		});

		it('removing the last alias clears the field rather than leaving {}', () => {
			binding.setResource('db', 'postgres');
			binding.removeResource('db');
			// We omit the field entirely when empty so the rendered
			// `WorkflowGraph` matches a freshly-loaded legacy doc — no
			// stray empty-map serialisation drift.
			expect(binding.graph.resources).toBeUndefined();
		});

		it('setResource re-typing an alias overwrites in place', () => {
			binding.setResource('store', 'postgres');
			binding.setResource('store', 's3_resource');
			expect(binding.graph.resources).toEqual({ store: 's3_resource' });
		});

		it('blank alias / type are rejected silently', () => {
			binding.setResource('', 'postgres');
			binding.setResource('db', '');
			binding.setResource('  ', 'openai');
			expect(binding.graph.resources).toBeUndefined();
		});

		it('alias / type are trimmed before storage', () => {
			binding.setResource('  api  ', '  openai  ');
			expect(binding.graph.resources).toEqual({ api: 'openai' });
		});

		it('two bindings on the same doc converge on resource changes', () => {
			const bindingB = new YjsGraphBinding(doc);
			binding.setResource('db', 'postgres');
			expect(bindingB.graph.resources).toEqual({ db: 'postgres' });
			bindingB.removeResource('db');
			expect(binding.graph.resources).toBeUndefined();
			bindingB.destroy();
		});
	});
