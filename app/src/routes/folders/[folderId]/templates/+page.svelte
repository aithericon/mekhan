<script lang="ts">
	...
	const folderId = $derived(ctx.folderId);

	// Templates homed in this folder (one-home model — not the old many-to-many
	// project membership). `recursive` stays false: only direct members here.
